...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const errors = validateAuditRequest(data);
    ...
    state.audit.push({
      ...
      website_url: sanitizeInput(data.website_url),
      audit_type: sanitizeInput(data.audit_type),
      goal: sanitizeInput(data.goal),
      created_at: new Date().toISOString()
    });
    writeStorage(state);

    return res.status(200).json({ success: true, message: 'Audit request submitted successfully' });
  } catch (error) {
